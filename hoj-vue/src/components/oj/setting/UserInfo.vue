  ...
  <div>
    ...
    <div class="section-main">
      ...
      <template v-if="!avatarOption.imgSrc">
        <el-upload
          ...
          :disabled="isProfileEditForbidden"
        >
          <div style="padding: 20px 0">
            <i class="el-icon-upload" :style="{color: isProfileEditForbidden ? '#ccc' : '#3399ff', fontSize: '52px'}"></i>
            <p :style="{color: isProfileEditForbidden ? '#999' : '#333'}">{{ $t('m.Upload_avatar_hint') }}</p>
            <p v-if="isProfileEditForbidden" style="color: #f56c6c; margin-top: 10px;">{{ $t('m.Forbidden_Modify_Profile') }}</p>
          </div>
        </el-upload>
      </template>
    ...
    <div style="text-align:center;margin-top:10px">
      <el-alert
        ...
        :title="$t('m.Profile_Edit_Forbidden')"
        type="warning"
        :closable="false"
...
export default {
  ...
  methods: {
    ...
    handleSelectFile(file) {
      if (this.isProfileEditForbidden) {
        myMessage.error(this.$i18n.t('m.Forbidden_Modify_Profile'));
        return false;
      }
    ...
    updateUserInfo() {
      if (!this.canModifyProfile) {
        myMessage.error(this.$i18n.t('m.Profile_Edit_Forbidden'));
        return;
      }
  ...
  computed: {
    ...
    canModifyProfile() {
      // 前端无法直接判断用户的数字角色ID，主要依赖后端权限检查
      // 后端会在changeUserInfo接口中检查权限并返回错误
      // 这里始终返回true，让用户可以看到表单，但保存时会由后端拦截
      return true;
    },
    isProfileEditForbidden() {
      const userInfo = this.$store.getters.userInfo;
      if (userInfo && userInfo.roleList) {
        return userInfo.roleList.includes('no_profile_edit_user') || userInfo.roleList.includes('no_profile_edit_mute_user');
      }
      return false;
    },
  },
